...
const FRONTEND_PORT = process.env.FRONTEND_PORT || 59542;
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    if (type === "etudiant_etranger") {
      ...
      // 🔥 MODIFICATION : Montants en TND
      const amount = subscriptionPlan === "monthly" ? 20 : 200;
...
app.post("/api/payment/init", async (req, res) => {
  try {
    ...
    // Montant en millimes (ex: 20 TND = 20000 millimes)
    const amountInt = Math.round(parseFloat(amount) * 1000);

    // Générer un ID de transaction unique
    const transactionId = `TXN_${Date.now()}_${crypto.randomBytes(4).toString("hex")}`;

    // 🔥 AJOUT : URLs de redirection
    const success_url = `http://localhost:${FRONTEND_PORT}/payment-success.html?transaction_id=${transactionId}&user_id=${userId}`;
    const cancel_url = `http://localhost:${FRONTEND_PORT}/payment-cancel.html`;

    const konnectData = {
      amount: amountInt,
      currency: "TND", // ← Changé en Dinar Tunisien
...
app.listen(PORT, () => {
  ...
  console.log(`📱 Frontend sur http://localhost:${FRONTEND_PORT}`);
});
